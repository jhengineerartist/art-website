...
export default function BlogPage({ params }: { params: { slug: string } }) {
    ...
    return (
        <main className="bg-enchilada-100">
            <article className='flex lg:flex-row max-lg:flex-col-reverse justify-center'>
                ...
                <figure className='max-lg:top-0 top-20 mx-auto w-1/3 max-h-screen lg:sticky max-lg:h-96 max-lg:w-full relative'>
                    <Image
                        className='relative object-contain object-top'
                        src={bp.heroImage}
                        alt={bp.title}
                        fill />
                    <figcaption className='relative'>Work Title</figcaption>
                </figure>
            </article>
        </main >
    )
